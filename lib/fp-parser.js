const { Ref, Def, resolveRefs } = require("./refs");

function parse(input, dontResolve) {
    ...
    let object = choice([
        sequence([literal("{"), ws, literal("}")], (data) => ({})),
        sequence(() => [literal("{"), members, literal("}")], (data) => data[1])
    ]);
    let refObjectDef = sequence([literal("&"), number, ws, object], (data) => {
        if (dontResolve) {
            return new Def(data[1], data[3]);
        } else {
            return refDict[data[1]] = data[3];
        }
    });
    let refArrayDef = sequence([literal("&"), number, ws, array], (data) => {
        if (dontResolve) {
            return new Def(data[1], data[3]);
        } else {
            return refDict[data[1]] = data[3];
    ...
    let member = sequence([ws, string, ws, literal(":"), element], (data) => ({[data[1]]: data[4]}));
    ...
    const result = json(input, 0);
    if (result) {
        if (dontResolve) {
            return result.value;
        } else {
            return resolveRefs(result.value, refDict, new Set());
        }
    } else {
...
function test() {
    ...
    for (let testCase of testCases) {
        const output = json(testCase, 0);
        if (output) {
            console.log(`"${testCase}" =`, JSON.stringify(output.value));
        } else {
